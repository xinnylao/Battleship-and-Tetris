    ...
    extends Agent
{
        ...
        extends Object
    {
        ...
        public Coordinate getNextCoordinateToExplore() {
            ...
            Map<Coordinate, Boolean> breezeMap = getKnownBreezeCoordinates();

            for (Coordinate frontierCoord : getFrontierPitCoordinates()) {
                int dirs[][] = new int[][]{{-1, 0}, {+1, 0}, {0, -1}, {0, +1}};
                double pitProbability = 0.0;
                boolean mustBeZero = false;
                for(int dir[] : dirs)
                {
                    int x = frontierCoord.getXCoordinate() + dir[0];
                    int y = frontierCoord.getYCoordinate() + dir[1];
                    Coordinate checkCoord = new Coordinate(x, y);
                    if (breezeMap.containsKey(checkCoord) && breezeMap.get(checkCoord) == true && mustBeZero == false) {
                        pitProbability += 0.25;
                    }
                    else if (breezeMap.containsKey(checkCoord) && breezeMap.get(checkCoord) == false && mustBeZero == false) {
                        pitProbability = 0;
                        mustBeZero = true;
                    }
                }
                if (pitProbability < lowestProb || (pitProbability == lowestProb && Math.random() < 0.5)) {
                    lowestProb = pitProbability;
                    toExplore = frontierCoord;
                    System.out.println("lowest prob is now: " + lowestProb);
                    System.out.println("toExplore is now: " + frontierCoord);
                }                
            }
            return toExplore;
        }  
